...
class CLIEngine {
    ...
    async lintFiles({
        ...
        // console.time(`iterate files: ${chunk} - ${chunkIndex}`);
        console.time(`lint files: ${chunk} - ${chunkIndex}`);

        // const files = [...fileEnumerator.iterateFiles(patterns)].slice(...chunk);

        // console.timeEnd(`iterate files: ${chunk} - ${chunkIndex}`);


        const result = await bluebird.map(files, async({ config: serializedConfig, filePath, ignored }) => {
            // const t0 = Date.now();
            ...
            // console.log("deserialize done", Date.now(), Date.now() - t0);

            // console.log("start lint", filePath, t0, chunkIndex);
            ...
            // console.log("read file", Date.now(), Date.now() - t0);
            ...
            // console.log("lint file", Date.now(), Date.now() - t0);

            // console.log("lint time", filePath, Date.now() - t0, chunkIndex);
            return result;
        }, {
            concurrency: 10
        });

        console.timeEnd(`lint files: ${chunk} - ${chunkIndex}`);
    ...
    async executeOnFiles(patterns) {
        ...
        const pool = workerpool.pool(`${__dirname}/cli-engine-worker.js`, {
            maxWorkers: chunksCount,
            workerType: "process",
            minWorkers: "max"
        });

        console.time("outer iterate files");
        const files = [...fileEnumerator.iterateFiles(patterns)].sort(() => Math.random() - 0.5);

        console.timeEnd("outer iterate files");

        console.log("iterate files done", Date.now(), Date.now() - t0);

        const chunkSize = Math.floor(files.length / chunksCount);
        const chunks = Array.from({ length: chunksCount }).map((x, i) => [i * chunkSize, i === (chunksCount - 1) ? files.length : (i + 1) * chunkSize]);

        const chunkResults = await Promise.all(chunks.map(async(chunk, chunkIndex) =>

            // flip to compare workers with same process
            (0 ? this.lintFiles({ chunk, patterns, chunkIndex, files: files.slice(...chunk) })

                : await pool.exec("lint", [{
                    params: { chunk, patterns, chunkIndex, files: files.slice(...chunk) },
