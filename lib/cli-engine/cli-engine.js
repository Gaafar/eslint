...
class CLIEngine {
    ...
    async lintFiles({
        ...
        const files = filesWithConfig || [...fileEnumerator.iterateFiles(patterns)].slice(...chunk);

        // console.timeEnd(`iterate files: ${chunk} - ${chunkIndex}`);

        const result = await Promise.all(files.map(async({ config, filePath, ignored }) => {
    ...
    async executeOnFiles(patterns) {
        ...
        const chunkResults = await Promise.all(chunks.map(async(chunk, chunkIndex) =>
            ...
            (0 ? this.lintFiles({ chunk, patterns, chunkIndex, filesWithConfig: files.slice(...chunk) })

                : await pool.exec("lint", [{
                    params: { chunk, patterns, chunkIndex },
